...
export const updateInvoice = async (req, res) => {
  try {
    ...
    if (updatedInvoice.status === 'DUE') {
      const previousInvoices = await Invoice.find({ clientId: updatedInvoice.clientId, status: 'DUE', _id: { $ne: invoiceId } });
      const totalOverdueAmount = previousInvoices.reduce((acc, invoice) => acc + (invoice.dueAmount || 0), 0);
      updatedInvoice.overdueAmount = totalOverdueAmount;
    }

    // Apply adjustments to invoice total
    if (adjustments && adjustments.length > 0) {
      const totalAdjustmentAmount = adjustments.reduce((acc, adjustment) => acc + adjustment.amount, 0);
